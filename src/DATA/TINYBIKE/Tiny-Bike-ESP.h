...
if (((BUTTON_DOWN)&&(ENDMAP_TBIKE!=1))&&((FREEAIR_TBIKE==0)||(Wheel_up_TBIKE<=2))&&(End_game_TBIKE==0) ) {
  if (ACCEL_TBIKE<32){ACCEL_TBIKE=ACCEL_TBIKE+0.30;}
  }else{
...
Tiny_Flip_TBIKE(1);
TIME_TRACK_TBIKE();
if (ENDMAP_TBIKE==1) {if (ACCEL_TBIKE<=1) {End_Line_Win_sound_TBIKE();Pause1_TBIKE();End_game_TBIKE=0;((void)0);goto NEXT_LEVEL;}}
if (End_game_TBIKE==1) {if (ACCEL_TBIKE<=1) {Pause1_TBIKE();((void)0);goto New_Games;}}
GAME_PLAY_TBIKE();
}}
void Pause1_TBIKE(void){
((void)0);My_delay_ms(2000);
}
...
if (Time_bar_Interval_TBIKE_timer<Time_bar_Interval_TBIKE) {Time_bar_Interval_TBIKE_timer++;
  ...
  Time_bar_Interval_TBIKE_timer=0;
  if (Time_bar_value_TBIKE<120) {Time_bar_value_TBIKE++;}else{End_game_TBIKE=1;}
  }
}

void PLANTAGE_TBIKE(void){
if (ACCEL_TBIKE>24){ACCEL_TBIKE=ACCEL_TBIKE-24;}else{PLANTAGE_0_TBIKE=0;Wheel_up_TBIKE=1;Pause_TBIKE=1;goto EED;}
