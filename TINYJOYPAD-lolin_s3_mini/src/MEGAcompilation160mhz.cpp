...
#include "DATA/TVBGONE/TVBGONE.h"
...
#define FLUID (1.5) //1.3 80mhz
#define FLUID_SETUP (1.1) //1.3 80mhz
#define Frame_Rate 15 //30 80 mhz
#define Frame_Rate_FADE 40 //30 80 mhz
#define SHOW_COMMUNITY 1

//Public Var
uint8_t MAX_VIGNETTE=22;// 16 for 17 games
//VAR SLIDE
...
// -----------------------------------------------------------------------------
// RGB EFFECT ENGINE
// One NeoPixel is used, so the effects are optimized for a single RGB LED.
// The effect task never blocks the main game/menu loop for long periods.
// -----------------------------------------------------------------------------
...
static uint8_t rgbEffect = RGB_RAINBOW;

uint32_t Wheel(uint8_t pos);
void setRGB(uint8_t r, uint8_t g, uint8_t b);
void rgbEffectStep(uint32_t now);
...
// Fast integer triangle wave: 0 -> 255 -> 0.
static uint8_t triWave(uint8_t x) {
  return (x < 128) ? (x << 1) : (255 - ((x - 128) << 1));
}

void rgbEffectStep(uint32_t now) {
  static uint16_t phase = 0;
  static uint32_t nextUpdate = 0;
  static uint8_t lastRandom = 0;
  static bool toggle = false;
  static uint8_t currentEffect = RGB_RAINBOW;
  static uint32_t effectEnd = 0;

  if ((int32_t)(now - nextUpdate) < 0) return;

  // Tự chọn một hiệu ứng mới ngẫu nhiên sau mỗi 2-6 giây.
  // Không chọn lại hiệu ứng vừa chạy để tránh cảm giác bị lặp.
  if ((int32_t)(now - effectEnd) >= 0) {
    uint8_t newEffect;
    do {
      newEffect = (uint8_t)random(0, RGB_COUNT);
    } while (RGB_COUNT > 1 && newEffect == currentEffect);

    currentEffect = newEffect;
    rgbEffect = currentEffect;
    phase = (uint16_t)random(0, 256);
    toggle = false;

    effectEnd = now + (uint32_t)random(2000, 6001);
  }
  ...
  switch (rgbEffect) {
    ...
    case RGB_RAINBOW:
      setRGB(
        (uint8_t)((Wheel((uint8_t)phase) >> 16) & 0xFF),
        (uint8_t)((Wheel((uint8_t)phase) >> 8) & 0xFF),
        (uint8_t)(Wheel((uint8_t)phase) & 0xFF)
      );
      phase += 2;
      delayMs = 18;
      break;
    ...
    case RGB_BREATH: {
      // Slowly breathe through several saturated colors.
      uint8_t level = triWave((uint8_t)(phase >> 1));
      uint32_t c = Wheel((uint8_t)(phase >> 2));
      r = ((c >> 16) & 0xFF) * level / 255;
      g = ((c >> 8)  & 0xFF) * level / 255;
      b = (c & 0xFF) * level / 255;
      setRGB(r, g, b);
      phase++;
      delayMs = 12;
    ...
    case RGB_COLOR_WIPE: {
      // One LED: this becomes a short pulse through the RGB wheel.
      uint8_t level = triWave((uint8_t)phase);
      uint32_t c = Wheel((uint8_t)(phase * 2));
      r = ((c >> 16) & 0xFF) * level / 255;
      g = ((c >> 8)  & 0xFF) * level / 255;
      b = (c & 0xFF) * level / 255;
    ...
    case RGB_RANDOM:
      // New bright random color every ~150 ms.
      lastRandom = (uint8_t)random(0, 256);
      r = (uint8_t)random(40, 256);
      g = (uint8_t)random(40, 256);
      b = (uint8_t)random(40, 256);
      // Bias one channel using the random wheel position for more vivid colors.
      if ((lastRandom & 3) == 0) r = 255;
      if ((lastRandom & 3) == 1) g = 255;
      if ((lastRandom & 3) == 2) b = 255;
      setRGB(r, g, b);
      delayMs = 150;
      break;

    case RGB_PARTY: {
      // Rapid alternating saturated flashes.
      static const uint8_t party[][3] = {
        {255, 0, 0}, {0, 255, 0}, {0, 0, 255},
        {255, 0, 255}, {0, 255, 255}, {255, 120, 0},
        {255, 255, 255}
      };
      uint8_t i = (phase >> 3) % 7;
      uint8_t brightness = (phase & 7) ? 255 : 80;
      setRGB(
        (uint16_t)party[i][0] * brightness / 255,
        (uint16_t)party[i][1] * brightness / 255,
        (uint16_t)party[i][2] * brightness / 255
      );
      phase++;
      delayMs = 45;
      break;
    ...
    case RGB_FIRE: {
      // Flickering warm orange/red effect.
      r = (uint8_t)random(150, 256);
      g = (uint8_t)random(15, 150);
      b = (uint8_t)random(0, 25);
      setRGB(r, g, b);
      delayMs = (uint16_t)random(25, 80);
      break;
    }

    case RGB_POLICE:
      // Red/blue alternating emergency-style flashes.
      if (phase & 1) {
        setRGB(255, 0, 0);
      } else {
        setRGB(0, 0, 255);
    ...
    default:
      rgbEffect = RGB_RAINBOW;
      phase = 0;
      delayMs = 20;
      break;
  }

  nextUpdate = now + delayMs;
}

void rainbowCycleTask(void *parameter) {
  (void)parameter;

  // Keep the LED animation independent from the OLED/menu/game loop.
  uint32_t now;
  while (true) {
    now = millis();

    // rgbEffectStep() tự động chọn ngẫu nhiên toàn bộ hiệu ứng.
    rgbEffectStep(now);

    // Small RTOS delay keeps CPU usage low while allowing very fast flashes.
...
void printSystemMonitor() {
  ...
  if (now - lastPrint >= 2000) {
    ...
    Serial.printf("Heap      : %d / %d KB (%.1f%%)\r\n", freeHeap / 1024, totalHeap / 1024, 100.0 * freeHeap / totalHeap);
    Serial.printf("PSRAM     : %d / %d KB (%.1f%%)\r\n", freePsram / 1024, totalPsram / 1024, 100.0 * freePsram / totalPsram);
    Serial.printf("Flash     : %d MB, %d MHz\r\n", flashSize / (1024 * 1024), flashSpeed);
...
if (yPASS<7) {return 0;}
uint8_t M10000_MEGA=(FIRMWARE_VERSION/1);
uint8_t M1000_MEGA=(((FIRMWARE_VERSION)-(M10000_MEGA*1))/1);
uint8_t M100_MEGA=(((FIRMWARE_VERSION)-(M1000_MEGA*1)-(M10000_MEGA*1))/1);
uint8_t M10_MEGA=(((FIRMWARE_VERSION)-(M100_MEGA*1)-(M1000_MEGA*1)-(M10000_MEGA*1)));
return 
(
