...
void TINYJOYPAD_UPDATE_TD(void) {
  ...
  if ((TimerDrop_TD < ((TINYJOYPAD_DOWN==0) ? /*((FreezeDrop_TD) ? 25 :*/ 0 /*)*/ : GameP_TD.Speed_TD))) {
    TimerDrop_TD++;
  } else {
...
uint8_t DrawNewPill_TD(uint8_t x, uint8_t y) {
  ...
  if (Pill_TD.Active) {
    if (Pill_TD.InTab) {
      uint8_t XX_TD = Xpos_TP + (Pill_TD.Gridx * 5);
      uint8_t YY_TD = Ypos_TP + (Pill_TD.Gridy * 5);
      Byteout |= blitzSprite(XX_TD, YY_TD, x, y, ReturncorectPills_TD(Pill_TD.PillPart1.Stat), pills_TD);
      if (Pill_TD.Vertical) {
        Byteout |= blitzSprite(XX_TD, YY_TD - 5, x, y, ReturncorectPills_TD(Pill_TD.PillPart2.Stat), pills_TD);
      } else {
        Byteout |= blitzSprite(XX_TD + 5, YY_TD, x, y, ReturncorectPills_TD(Pill_TD.PillPart2.Stat), pills_TD);
      }
    } else {
      Byteout |= blitzSprite(Pill_TD.GraphPosX, Pill_TD.GraphPosY, x, y, ReturncorectPills_TD(Pill_TD.PillPart1.Stat), pills_TD);
      if (Pill_TD.Vertical) {
        Byteout |= blitzSprite(Pill_TD.GraphPosX, Pill_TD.GraphPosY - 5, x, y, ReturncorectPills_TD(Pill_TD.PillPart2.Stat), pills_TD);
      } else {
        Byteout |= blitzSprite(Pill_TD.GraphPosX + 5, Pill_TD.GraphPosY, x, y, ReturncorectPills_TD(Pill_TD.PillPart2.Stat), pills_TD);
      }
    }
  }
  return Byteout;
...
display.buffer[(x+(y*128))]=(((Show_)?~SPEED_BLITZ(41, 6, x, y, 0, shad_TD):0xff)&pgm_read_byte(&Introgame_TD[x + (y * 128)]));
    }
  }
display.display();
}
...
void TinyFlip_TD(void) {
  for (uint8_t y = 0; y < 8; y++) {
    for (uint8_t x = 0; x < 128; x++) {
 display.buffer[(x+(y*128))]=(DrawDisplayLV_TD(x, y) | Draw_Score_TD(x, y) | DrawLoupe_TD(x, y) | DrawPreviewPill_TD(x, y) | DrawTAB_TP(x, y) | DrawNewPill_TD(x, y) | ((~DrawShadowPreviewPill_TD(x, y) & pgm_read_byte(&background_TD[x + (y * 128)]))));
    }
  }
...
void LevelClearedFlip_TD(void) {
  for (uint8_t y = 0; y < 8; y++) {
    ...
    for (uint8_t x = 0; x < 128; x++) {
display.buffer[(x+(y*128))]=(DrawDisplayLV_TD(x, y) | Draw_Score_TD(x, y) | DrawLoupe_TD(x, y) | Draw_Score_TD(x, y) | SPEED_BLITZ(51, 2, x, y, 0, LevelCleared_TD) | (pgm_read_byte(&background_TD[x + (y * 128)])));
    }
  }
display.display();
}