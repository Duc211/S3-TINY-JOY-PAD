...
uint8_t *getBuffer(void)
{
  ...
  void FPS_Temper(void) {
    while ((curtMil - MemMil) < FPS_Break) { curtMil = millis(); }
    MemMil = curtMil;
  }

  void Init(SSD1306 *disp_) {
    Disp_1 = disp_;
  }
  void clear(void) {
fillScreen(BLACK);
  ...
  void drawSelfMasked(int8_t x_, int8_t y_, uint8_t const *PRGM_, uint8_t F_) {
    ...
    for (int16_t y = STARTY; y < endY; y++) {
      ...
      for (int16_t x = x_; x < (WSPRITE_ + x_); x++) {
        ...
        Disp_1->buffer[x + (y << 7)] |= ESP_blitzSprite(x_, y_, HSPRITE_, x, y, F_, PRGM_);
      }
    }
  }
  ...
  void drawOverwrite(int8_t x_, int8_t y_, uint8_t const *PRGM_, uint8_t F_) {
    ...
    for (int16_t y = STARTY; y < endY; y++) {
      ...
      for (int16_t x = x_; x < (WSPRITE_ + x_); x++) {
        ...
        Disp_1->buffer[x + (y << 7)] &= BlackSquare(y_, y, PRGM_);
        Disp_1->buffer[x + (y << 7)] |= ESP_blitzSprite(x_, y_, HSPRITE_, x, y, F_, PRGM_);
      }
    }
  }
