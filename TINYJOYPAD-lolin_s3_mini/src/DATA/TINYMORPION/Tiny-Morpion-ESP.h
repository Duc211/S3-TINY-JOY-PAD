...
uint8_t BOARD[3][3];
uint8_t Players[2];
uint8_t MyTurn; //0=cpu_PLAY or Player2_PLAY, 1=player1_PLAY
uint8_t POS_XY[2];
uint8_t WinPLY;
uint8_t WinCPU; // Dùng làm điểm số cho Người chơi 2 trong chế độ 2 người
uint8_t ENDGAME;
uint8_t SELECT=1;
uint8_t GAME_MODE=0; // 0: Chế độ đấu với CPU, 1: Chế độ 2 người chơi
...
// Xác định chế độ chơi dựa trên vị trí SELECT ở menu
// SELECT == 2 tương ứng với tùy chọn thêm (Chế độ 2 người chơi)
if (SELECT == 2) {
  GAME_MODE = 1; // 2 người chơi
} else {
  GAME_MODE = 0; // Đấu với máy (CPU) theo độ khó SELECT tương ứng
}
...
if (GAME_MODE == 1) {
  ...
  if ((No_Ripple==0)&&(BUTTON_DOWN)) {
    if (BOARD[POS_XY[1]][POS_XY[0]]==2) {
      uint8_t CurrentPlayer = (MyTurn == 1) ? Players[1] : Players[0];
      BOARD[POS_XY[1]][POS_XY[0]] = CurrentPlayer;
      REFRESH_SCREEN_TMORPION();
      SND_BOX_TMORPION(CurrentPlayer);
      Check_WIN_TMORPION(CurrentPlayer);
      MyTurn = !MyTurn; // Đổi lượt giữa Người chơi 1 và Người chơi 2
      No_Ripple = 1;
    }
  }
} else {
  // --- CHẾ ĐỘ ĐẤU VỚI MÁY (GỐC) ---
  if (MyTurn) {
    if (TINYJOYPAD_RIGHT==0) {
      DEBOUNCE(TINYJOYPAD_RIGHT==0) {
        if (No_Ripple==0) {No_Ripple=1;POS_XY[0]=(POS_XY[0]<2)?POS_XY[0]+1:2;}
...
uint8_t activePlayer = (GAME_MODE == 1) ? ((MyTurn == 1) ? Players[1] : Players[0]) : Players[MyTurn];
for (t=0;t<9;t++){if (p[t]==activePlayer) {p[t]=2;}}
REFRESH_SCREEN_TMORPION();
My_delay_ms(24);
...
if (GAME_MODE == 1) {
  // Xử lý điểm số cho chế độ 2 người chơi
  uint8_t winningTurn = (MyTurn == 1) ? 1 : 0;
  if (winningTurn == 1) {
    if (WinPLY < 9) { WinPLY++; } else { SND_BOX_TMORPION(4); ENDGAME = 2; TMP_OUT = 1; }
  } else {
    if (WinCPU < 9) { WinCPU++; } else { SND_BOX_TMORPION(5); ENDGAME = 1; TMP_OUT = 1; }
  }
  MyTurn = 1;
} else {
  switch (MyTurn){
...
if (GAME_MODE == 1) {
  DIVIDE_BY_2 = 0; // Luôn hiển thị con trỏ bình thường cho cả 2 người chơi
} else {
